...
def get_style_instruction(style):
    if style == "간결형":
        ...
        - 공문, 안내문, 의뢰문처럼 쓰지 말 것
        - '~해줘', '~작성해줘', '~정리해줘' 형태로 자연스럽게 끝낼 것
        - 명령형이 아니라 요청형으로 작성할 것
        - 공문체 금지
        """
    elif style == "초간결형":
...
- 아래 4개 항목을 반드시 정확히 이 순서로 출력할 것:
  1. 역할
  2. 목표
  3. 조건
  4. 출력 형식

- 각 항목은 반드시 아래 형식을 그대로 사용할 것:
  "번호. 항목명" (예: 2. 목표)

- 번호와 항목명은 반드시 같은 줄에 작성할 것
  (절대 줄바꿈으로 분리하지 말 것)

- 잘못된 형식 예시는 절대 사용하지 말 것:
  ❌ 2.
     목표
  ❌ 2. 
     목표

- 각 항목 사이에는 반드시 한 줄만 띄울 것

- 항목 제목 뒤에는 줄바꿈 후 내용 작성

- 형식을 임의로 변경, 생략, 변형하지 말 것
"""
